  ...
  renderLayerMenu = () => {
    const menuList = rasterLayers =>
      rasterLayers.map(
        rasterLayer =>
          rasterLayer.status ? (
            <a href="#" className="active" onClick={() => this.layerSelectedDeactivate(rasterLayer)}>
              {' '}
              {rasterLayer.name}
            </a>
          ) : (
            <a href="#" onClick={() => this.layerSelectedActive(rasterLayer)}>
              {' '}
              {rasterLayer.name}
            </a>
          )
      );
    return <div>{menuList(rasterLayersToDisplay)}</div>;
  };
